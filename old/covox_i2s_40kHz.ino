...
void IRAM_ATTR onTimer() {
  ...
  int16_t sample = value - 128;
  //int16_t sample = (value - 128)<<8;
  buf[i] = (sample<<24) | (sample<<8); 
  //buf[i] = (sample<<16) | (sample<<0); 
  //buf[i] = (value<<16) | (value<<0); 
...
static const i2s_pin_config_t pin_config = {
    .bck_io_num = 26,//26,                                 // The bit clock connectiom, goes to pin 27 of ESP32
    .ws_io_num = 25,//25,                                  // Word select, also known as word select or left right clock
    .data_out_num = 27,//22,                               // Data out from the ESP32, connect to DIN on 38357A
...
void setup() {
  ...
  //timer = timerBegin(0, 3, true);
  timerAttachInterrupt(timer, &onTimer, true);
  timerAlarmWrite(timer, 25, true); // 25 -> 40kHz, 50 -> 20kHz, 10 -> 100kHz
